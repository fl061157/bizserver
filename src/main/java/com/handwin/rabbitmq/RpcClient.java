...
public class RpcClient {
    ...
    static String LOCAL_IP = System.getProperty("com.handwin.localip");
    ...
    protected DefaultConsumer setupConsumer() throws IOException {
        ...
        try {
            _channel.queueDeclare(String.format("rabbitmq.reply-to_%s", LOCAL_IP), true, false, false, null);
        } catch (Exception e) {
            e.printStackTrace();
        }
        _channel.basicConsume(String.format("rabbitmq.reply-to_%s", LOCAL_IP), true, consumer);
        return consumer;
    }
    ...
    public byte[] primitiveCall(AMQP.BasicProperties props, byte[] message)
        ...
        synchronized (_continuationMap) {
            ...
            props = ((props == null) ? new AMQP.BasicProperties.Builder() : props.builder())
                    .correlationId(replyId).replyTo(String.format("rabbitmq.reply-to_%s", LOCAL_IP)).build();
